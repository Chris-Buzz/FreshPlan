...
import { ShoppingCart, Check, Loader2, DollarSign, RefreshCw } from 'lucide-react';
    ...
    <div className="space-y-6 pb-24">
      <div className="flex justify-between items-center">
        ...
        <button 
            ...
            disabled={isLoading}
            className="text-emerald-600 p-2 hover:bg-emerald-50 rounded-full transition"
            title="Refresh List"
        >
      ...
      {items.length === 0 ? (
        ...
        <>
            <div className="bg-emerald-900 text-white p-6 rounded-xl shadow-lg flex justify-between items-center">
                <div>
                    <div className="text-emerald-200 text-sm font-medium mb-1">Estimated Cost</div>
                    <div className="text-3xl font-bold flex items-center">
                        <DollarSign className="w-6 h-6 text-emerald-400" />
                        {totalCost.toFixed(2)}
                    </div>
                    <div className="text-xs text-emerald-400 mt-1">Based on local averages</div>
                </div>
                <div className="h-12 w-12 bg-emerald-800 rounded-full flex items-center justify-center">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 divide-y divide-gray-100">
                {items.map((item) => (
                    ...
                    >
                        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition ${item.checked ? 'bg-emerald-500 border-emerald-500' : 'border-gray-300'}`}>
                        ...
                        <div className={`font-semibold text-sm ${item.checked ? 'text-gray-300 line-through' : 'text-gray-700'}`}>
